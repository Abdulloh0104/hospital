...
export class AuthStaffService {
  constructor(
    private readonly usersService: StaffService,
    private readonly jwtService: JwtService
  ) {}

  async generateTokens(user: Staff) {
    const payload = {
      id: user.id,
      email: user.email,
      role: user.role,
  ...
  async signIn(signInDto: SignInDto, res: Response) {
    const user = await this.usersService.findStaffByEmail(signInDto.email);
    if (!user) {
  ...
  async signOut(refreshToken: string, res: Response) {
    ...
    const response = {
      message: "Staff logged out seccessfully",
    };
    return response;
  }
